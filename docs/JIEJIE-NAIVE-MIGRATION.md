...
在 `route.rules` 中，**在 `resolve` 之后、`ip_cidr` 拒绝规则之前**，
加入 VPS 自身地址的最小服务例外，并把自身地址并入拒绝列表：
...
例外必须同时满足四个条件（inbound=naive-in、network=tcp、
...
- **多网卡 / 附加公网 IP / NAT 入口**：每一个都可能成为客户端可达的目标，
  需要分别列出。
- **必要业务例外**：如果确有客户端需要回连 VPS 公网地址上的其他服务，
  在该 reject 规则**之前**，用与 2222 相同的四个条件（inbound + network +
